...
`LowestArmyTask` showed the trunk linearly preserves lowest-army; this is the
direct follow-up — can a fresh head on the frozen trunk beat the 47.1%
lowest-army RULE at predicting the ACTUAL next-death victim? The dataset already
labels the victim (`next_elim_target`), so this reuses the shared cross-player
heads + masked CE and only swaps the target.

The headline is `top1_imminent` (dt<10): the aggregate is buoyed by the easy
2-3-alive endgame, so the contested imminent regime is where the rule (47.8%)
actually has to be beaten (6.13-15). "Early-stop" is free — the summary reports
...
overfit.
  ...
  caps at ~47% even here -> who-dies-next ~= the leaderboard, the head was
    near-optimal, and its unique trunk contribution is ~nil (insight 4 / weakens
    the 24k-transfer bet).
"""
...
def main() -> None:
    ...
    run_probe_cli(lambda _cfg: WhoDiesNextTask(), args)
