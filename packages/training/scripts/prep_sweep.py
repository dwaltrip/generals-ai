...
from training.settings import SWEEPS_DIR
...
def cmd_init(args: argparse.Namespace) -> None:
    ...
    print()
    print("Then run:")
    sweep_rel = sweep_dir.relative_to(PROJECT_ROOT)
    print(f"  ./packages/training/scripts/prep_sweep.py generate {sweep_rel}")
...
def cmd_generate(args: argparse.Namespace) -> None:
    ...
    write_executable(
        ...
        render_cloud_script(sweep_rel, cells),
    )
    write_executable(
        sweep_dir / "run-local.sh",
        render_local_script(sweep_rel, cells),
    )
    write_executable(
        sweep_dir / "fetch.sh",
        render_fetch_script(sweep_rel),
    )

    print(f"Generated {len(cells)} cell configs in {configs_dir.relative_to(PROJECT_ROOT)}/")
    print("Generated run-cloud.sh, run-local.sh, fetch.sh")
    print()
    print("Next steps:")
    print(f"  1. Review configs in {configs_dir.relative_to(PROJECT_ROOT)}/")
...
def render_cloud_script(sweep_rel: str, cells: list[tuple[str, str]]) -> str:
...
def render_local_script(sweep_rel: str, cells: list[tuple[str, str]]) -> str:
    cells_lines = "\n".join(f'  "{label}:{config_rel}"' for label, config_rel in cells)
...
# (run_bc_local.py doesn't print run_dir). Check data/training/runs/
...
def render_fetch_script(sweep_rel: str) -> str:
    return f"""\
#!/usr/bin/env bash
# Auto-generated by prep_sweep.py — safe to regenerate.
#
