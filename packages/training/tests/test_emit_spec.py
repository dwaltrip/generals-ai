...
from training.bc.emit_spec import EmitSpec, emit_spec_for_model, emit_spec_from
from training.bc.model_config import build_model_cfg


def test_emit_spec_validation():
    arch = build_model_cfg(elim_head_variant="time_bin")
    with pytest.raises(ValueError, match="time_bin requires alive_mask"):
        EmitSpec(
            obs=arch.obs,
...
def test_emit_spec_from(variant, requested, expect_alive):
    ...
    spec = emit_spec_from(arch, MetricsConfig(include_alive_mask=requested), frame_info=True)
    assert spec.alive_mask is expect_alive
    assert spec.obs is arch.obs
    assert spec.targets == targets_cfg_from(arch)
    assert spec.frame_info is True
    assert spec.sim_frame is False
...
def test_emit_spec_for_model(variant, expect_alive):
    spec = emit_spec_for_model(build_model_cfg(elim_head_variant=variant), frame_info=False)
    assert spec.alive_mask is expect_alive
