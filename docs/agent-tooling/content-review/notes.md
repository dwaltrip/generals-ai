...
**Style grounding recalibrated; calibration anti-pattern added (2026-07-07).** Two changes out of the writing-calibration exercise (session 2026-07-06/07). The active-voice example ranked "the hook denies the call" above "the call is denied by the hook" — backwards for a passage tracking calls; replaced with topic-based guidance (subject = the noun the passage is tracking). And a survey of the first six reports showed the reviewer reliably applying checkable style rules but producing almost no calibration findings — the lens had no named entry for overclaim. Added the "claim outruns its shown backing" anti-pattern, framed as checkability (is the backing shown on the page?) so it stays inside the legibility lens and the blinding rather than asking the reviewer to judge truth.
