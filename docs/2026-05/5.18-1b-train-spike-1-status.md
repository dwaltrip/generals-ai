**Note:** This was converted from an evergreen doc to a session note. I used doc date and number `5.18-1b` as that put it the correct position chronologically. The `1b` has no other significance - it's purely for ordering the doc by when it was first created.

# Training Spike 1 — Status

Working guide: [`5.17-4-training-spike-plan.md`](./5.17-4-training-spike-plan.md). See [§3](./5.17-4-training-spike-plan.md#3-the-plan) for the phase-by-phase sketches.

Chunk D sub-step spec: [`5.18-3-phase-1d-spec-and-pass-head-audit.md`](./5.18-3-phase-1d-spec-and-pass-head-audit.md). All sub-steps shipped; the doc is historical now.
